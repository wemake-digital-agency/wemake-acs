document.addEventListener("DOMContentLoaded", function () {
  document.addEventListener("click", function (event) {
    if (event.target.classList.contains("acs-button")) {
      const acsElement = document.querySelector("#wemake-acs");
      if (acsElement) {
        acsElement.classList.toggle("active");
      } else {
        console.error("Element with ID 'wemake-acs' not found.");
      }
    }
  });
});
...
jQuery(function ($) {
  ...
  let acsAction,
    body = $("body"),
    zoomElements = $(
      "body,h1,h2,h3,h4,h5:not('.acs-title'),h6:not('.acs-title'),p:not('.acs-wemake'),li:not('.acs'),a:not('.acs-link'):not(.acs-powered):not(.wm-poweredby-logo),span,div.icon-item_text,.fr-text"
    ),
    zoomStep = 1.1,
    maxSize = 200,
    minSize = 10,
  ...
  if (typeof sessionStorage !== "undefined") {
    ...
    if (acs_body_class !== null) {
      ...
      for (let ii = 0; ii < acs_body_class.length; ii++) {
        ...
        $(".acs-link[data-action=" + acs_body_class[ii] + "]").addClass(
          "active"
        );
      }
    }
